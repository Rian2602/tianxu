...
- **Fix temuan audit ChatGPT** (plan `docs/superpowers/plans/2026-08-14-fix-temuan-audit-chatgpt.md`, Batch 6 di `list_bug.md`): **A1** hang EXP ranah tertinggi di-cap (0.00s, sebelumnya hang ratusan detik); **A2+A3** lapisan required node — objective `talk` opsional `node`/`nodes`/`start_node`, dialog `start(forced_node)` + catat `visited`, konfrontasi 3aa terjadi SAAT quest berjalan (test bug-lock ditulis ulang), validator aturan 4; **A5** `advance_time` bandingkan waktu absolut (overshoot memenuhi); **A6** `skill_pool` semua elemen; **A7** main `defeat` filter `enemies`; **A8** fallback hardcode hunt dihapus (murni `config.world.hunt`).
- **Polish web** (plan yang sama): hierarchy visual + semantic color (B1/B2), ranah hero stat + progress exp statis (B3), log speaker + separator scene (B4), dialog story card (B5), mobile naratif + drawer quest/inventori (B6), ornamen title SVG inline (B7), `:focus-visible` + focus trap modal + Escape (B8), feedback loading `is-loading` (B9). Tekstur aset pengguna (`web/static/assets/textures/`: paper-dark, silk-dark, ink-wash, cloud-mist, gold-noise) dipakai halus via overlay gelap — tetap dalam batas GDD §12.5 (tanpa animasi/audio).
- **C4 — ambience lokasi data-driven**: field opsional `ambience` per lokasi ∈ enum `config.world.ambiences` (`academy`/`forest`/`night`/`market`/`hall`/`library`) → gradient/tekstur latar statis web (`body.ambience-<x>`); `view().location.ambience`; validator aturan 14 tolak enum tak dikenal.
- **Defer C1-C3 plan (eksekusi, 2026-08-14)**: **C1 font self-host** — Noto Serif SC (cerita/`--serif`) + Noto Sans SC (HUD/`--sans`) subset woff2 OFL (Latin + 16 Hanzi game; self-host offline di `web/static/assets/fonts/`, ~1.4MB, `@import fonts.css`); **C2 icon Lucide** — 15 SVG self-host (ISC, `LICENSE.lucide.txt`) inline async via `icon()` di stat & tombol (bukan emoji, keputusan "tanpa emoji" dipertahankan); **kompresi tekstur** — 5 PNG downscale 2× + zlib 9 (stdlib-only): **14MB → 3MB** (PNG RGB valid, CRC OK); `web/app.py::do_HEAD` — header-only untuk file statis (RFC 9110, sebelumnya 501).
- Dokumentasi: `docs/list_bug.md` Batch 6 (A1-A8, B1-B9, C4), plan `docs/superpowers/plans/2026-08-14-fix-temuan-audit-chatgpt.md`, sinkronisasi ENGINE_ARCHITECTURE / README / PROJECT.

### Changed
- Jumlah test: 251 → **266**; validator tetap 16 aturan exit 0 (aturan 4 diperluas: objective `talk` node wajib; aturan 14: `ambience` enum config).
- `README.md` & `PROJECT.md`: jumlah test disinkronkan (266).
