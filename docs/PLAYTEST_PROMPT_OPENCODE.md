...
- **Side quest tersedia sejak hari 1** (bukan hari 2) — verifikasi penawaran hari 1 dari Pemburu/Su Qing/Mo Yun.
- **Equip** (`pasang <senjata>`) & **gate battle** harus berfungsi.
- **Kompanion** (jalur Summoning): auto-attack, target musuh, KO → istirahat.
- Web UI: `python3 web/app.py` → `http://127.0.0.1:8000` — menu Mulai Baru/Lanjut, 3 kolom, panel Tianyuan Ling, battle.

## 7. Bukti wajib di akhir laporan

1. Output `python3 tools/validate_data.py`.
2. Output `python3 -m pytest tests/ -q`.
3. Isi `saves/save_penuh.json` dan `saves/save_kedua.json` (stat, quest selesai, inventori, ingatan) — termasuk **level akhir** (harus 4–6).
