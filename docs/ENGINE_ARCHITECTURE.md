...
- **`arcs[].endings` (C3, 2026-08-14)**: scaffold ending data-driven (GDD §3.4/§9 — moralitas → penentu ending) — tiap arc boleh punya `endings` opsional: `{id, title, desc, condition}`; `view().arc_summary.ending` = ending **pertama yang kondisinya cocok** (first-match AND, `_eval_condition` dipakai ulang) atau `None` (arc tanpa endings — arc akademi **tanpa endings**, kontrak view lama utuh); CLI banner & modal web menampilkan ending bila ada. 3 ending tematik (Reformer/Destroyer/Ascetic) = data arc final. Validator aturan 7: id unik, title string, condition hanya kunci kondisi dialog yang dikenal (`_check_dialog_condition` — `mood_min` dsb. ditolak).
...
| 7 | `config.json`: starting quest ada, akademi valid, referensi `element_advantage` valid; `arcs` valid (final_quest ada, memories_total > 0, branches non-kosong, endings opsional: id unik/title string/condition hanya kunci kondisi dikenal — C3); `world.safe_fallback_location` valid & aman (B1/B2); `time.month_length_days` int > 0 + `month_names` 12 (C2); kondisi dialog `month_min/max` int 1..12 | `config.starting.current_quest tidak ditemukan` / `config.arcs: final_quest ...` |
...
- **Ending** (disahkan): 3 tematik; mekanisme penentu final (bobot pilihan kunci + moralitas) dijabarkan lebih rinci saat konten arc final. **Scaffold data-driven (C3, 2026-08-14)**: `config.arcs[].endings` (first-match AND via `_eval_condition`) → `arc_summary.ending`; arc akademi tanpa endings (non-breaking), arc final tinggal isi data 3 ending tematik (GDD §9).
...
- **Tahap C — selesai (2026-08-14, plan `2026-08-14-rampungkan-arc-akademi-tahap-c.md`)**: (C1) teknik **dipelajari & ditingkatkan** (GDD §7) — efek `technique` (quest/dialog reward), `player.techniques`/`technique_levels` (save round-trip), aksi `upgrade_technique` (titik aman, biaya `technique_upgrade_cost_base` × level, batas `order` ranah + 1), power scaling `× (1 + (level−1) × technique_power_growth_per_level)`, CLI `tingkatkan <teknik>` + panel web; (C2) **bulan derived** (GDD §7) — `state.month`/`month_name`, kondisi dialog `month_min/max`, header CLI/web; (C3) **moralitas → ending scaffold** (GDD §3.4/§9) — `arcs[].endings` data-driven → `arc_summary.ending`. Validator: `EFFECT_TYPES` + `technique` (aturan 13), config upgrade/time/endings (aturan 7), kondisi dialog kunci tak dikenal ditolak; jumlah aturan tetap 16. **250 passed**, validate exit 0.
- **Playtest putaran 2 — observasi → KEPUTUSAN (disahkan 2026-08-14, Tahap A plan `2026-08-14-rampungkan-arc-akademi-tahap-a.md`)** — tidak ada lagi item "open":
  ...
  - **Side quest "kalahkan" tanpa lapor → objektif defeat kini punya `report_to`**: `q_side_berburu` butuh lapor ke `npc_pemburu` setelah 2 kill (`quest.py::notify_dialog_ended` memeriksa `defeat + report_to`; validator aturan 2 memeriksa referensi npc). `objective_text` menampilkan status lapor (✓/—) — ✓ hanya bila target kill sudah penuh DAN sudah lapor (lapor sebelum kill = bukan laporan).
