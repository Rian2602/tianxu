...
>
> **STATUS EKSEKUSI (2026-08-14 lanjutan)**: 🔄 **C1 & C2 SELESAI (commit di bawah)** — C3 belum dimulai. Verifikasi: validate exit 0 + **248 passed** (+4 test C2: view month derived, kondisi dialog month, 2 validator). Deviasi C1 (tercatat saat eksekusi): (1) batas level upgrade = **`order` ranah + 1**, bukan `technique_slots` — `technique_slots` ranah awal = 1 tak memberi ruang upgrade (fitur mati); order+1 memberi 1× upgrade di ranah awal, tetap data-driven; (2) test power scaling memakai **keanggotaan** daftar panggilan `_calc_damage` (giliran musuh menimpa panggilan terakhir). Deviasi C2: formula month = **`(day−1) // mld + 1`** (day 1..30 = Bulan 1) — formula plan `day // mld + 1` off-by-one di kelipatan persis (day 30 → Bulan 2); kasus plan day 1/31/61 tetap terpenuhi.
