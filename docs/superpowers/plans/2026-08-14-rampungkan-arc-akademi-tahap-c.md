...
>
> **STATUS EKSEKUSI (2026-08-14 lanjutan)**: ✅ **SELESAI — C1, C2, C3** (commit `b0abbc3` C1, `77188eb` C2, C3 di bawah). Verifikasi akhir: validate exit 0 + **250 passed**; playthrough CLI 3 akademi hijau (non-breaking). Deviasi C1: (1) batas level upgrade = **`order` ranah + 1**, bukan `technique_slots` — slots ranah awal = 1 tak memberi ruang upgrade (fitur mati); (2) test power scaling memakai **keanggotaan** daftar panggilan `_calc_damage`. Deviasi C2: formula month = **`(day−1)//mld+1`** (plan `day//mld+1` off-by-one di kelipatan persis). Deviasi C3: blok `return` `view()` sempat tersangkut di `_pick_ending` (unreachable) saat implementasi awal — dikoreksi sebelum commit (test menangkap); `_good()` test validator ditambah `arcs` valid.
