...
>
> **STATUS EKSEKUSI (2026-08-14 lanjutan)**: 🔄 **C1 SELESAI (kode + test hijau, commit di bawah)** — C2 & C3 belum dimulai. Verifikasi C1: validate exit 0 + **244 passed** (+5 test: efek technique, upgrade session, power scaling battle, 2 validator). Deviasi (tercatat saat eksekusi): (1) batas level upgrade = **`order` ranah + 1**, bukan `technique_slots` — `technique_slots` ranah awal (`realm_pengumpul_qi`) = 1 sehingga slots tak memberi ruang upgrade sama sekali (fitur mati); order+1 memberi 1× upgrade di ranah awal & naik per ranah, tetap data-driven (realms.csv); (2) test power scaling memakai **keanggotaan** daftar panggilan `_calc_damage`, bukan panggilan terakhir — giliran musuh (attack 0) menimpa nilai teknik bila hanya disimpan panggilan terakhir.
